    ...
    <v-card style="padding: 10px" elevation="3">
      <v-row style="margin-bottom: 5px">
        ...
        <v-col cols="12" offset-md="1" md="4">
          <image-handler single :disabled="!editable" style="margin-top: 15%" v-model="image"></image-handler>
        </v-col>
      </v-row>
      <v-divider></v-divider>
      <v-row style="margin-top: 5px">
        <v-col cols="12" md="4">
          <v-text-field label="Nombre(s)" solo :readonly="!editable" :disabled="myUser.accessLevel === 3" v-model="firstName"></v-text-field>
        </v-col>
        <v-col cols="12" md="4">
          <v-text-field label="Apellido(s)" solo :readonly="!editable" :disabled="myUser.accessLevel === 3" v-model="lastName"></v-text-field>
        </v-col>
        <v-col cols="12" md="4">
          <v-menu ref="menu" v-model="menu" :close-on-content-click="false" :return-value.sync="fNacimiento" transition="scale-transition" offset-y disabled min-width="290px">
            ...
            <v-date-picker v-model="fNacimiento" no-title scrollable locale="es">
              <v-spacer></v-spacer>
      ...
      <v-row>
        <v-col cols="12" md="4">
          <v-text-field label="Telefono" solo type="number" :readonly="!editable" v-model.number="phone"></v-text-field>
        </v-col>
        <v-col cols="12" md="4">
          <v-text-field label="Correo Institucional" solo readonly :disabled="myUser.accessLevel === 3" type="email" v-model="email"></v-text-field>
        </v-col>
        <v-col cols="12" md="4">
          <v-text-field label="Correo Personal" solo :readonly="!editable" v-model="emailPersonal"> </v-text-field>
        </v-col>
        <v-col cols="12" md="4">
          <v-text-field label="CURP" solo :readonly="!editable" :disabled="myUser.accessLevel === 3" v-model="curp"> </v-text-field>
        </v-col>
        <v-col cols="12" md="4">
          <v-text-field label="Estado" solo :readonly="!editable" v-model="state"> </v-text-field>
        </v-col>
        <v-col cols="12" md="4">
          <v-text-field label="Ciudad" solo :readonly="!editable" v-model="city"> </v-text-field>
        </v-col>
        <v-col cols="5" md="5">
        ...
        <v-col cols="12" md="12">
          <v-text-field label="Direccion" solo :readonly="!editable" v-model="dir"> </v-text-field>
        </v-col>
      </v-row>
      <v-divider></v-divider>
      <v-card-actions>
