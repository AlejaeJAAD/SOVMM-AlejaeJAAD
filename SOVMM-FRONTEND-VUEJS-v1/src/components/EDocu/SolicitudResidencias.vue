  ...
  <v-card flat style="margin-bottom: -0.5rem">
    <v-form>
      <v-row>
        <v-col cols="12">
          <a @click="styleButton" style="color: inherit; text-decoration: none;"
             href="https://sovmm-03.s3.us-east-2.amazonaws.com/FORMATO-SOLICITUD+DE+RESIDENCIAS+PROFESIONALES.pdf"
             target="_blank">
            <v-btn
              large
              dark
            ...
            >
              Descarga el formato de solicitud de residencias profesionales
              <v-icon class="ml-2">mdi-download</v-icon>
            </v-btn>
          </a>
          <form action="https://sovmm-03.s3.us-east-2.amazonaws.com/FORMATO-SOLICITUD+DE+RESIDENCIAS+PROFESIONALES.pdf"
                method="get">
          </form>
        </v-col>
        <v-col cols="12">
          <v-card-subtitle>
            Agrega tu formato de solicitud de residencias profesionales
            <v-icon>mdi-arrow-down-bold</v-icon>
          </v-card-subtitle>
          <document-handler single v-model="solicitudRes"></document-handler>
        </v-col>
      </v-row>
      <v-card-actions>
        <v-spacer></v-spacer>
      ...
      <v-card flat style="background-color: #333333"
        ...
        CARGAR FORMATO DE SOLICITUD DE RESIDENCIAS PROFESIONALES
        CORRESPONDIENTE PARA SU ACTUALIZACION
      </v-card>
      <v-row>
        <v-col cols="12">
          <v-card flat
                  v-for="document in this.info.solicitudRes" :key="document">
            <pdf :src="document"
                 :page="currentPage"
...
export default {
  ...
  methods: {
    ...
    postDocument() {
      ...
      console.log(this.proy.solicitudRes, '49494');
      /*
      const data = {
        document: '',
      };
      ...
      */
      if (this.solicitudRes.length !== 0) {
        ...
        this.isLoading = true;
        const fD = new FormData();
        for (let i = 0; i < this.solicitudRes.length; i += 1) {
          fD.append(`document${i}`, this.solicitudRes[i]);
        }
        Axios.post('documents', fD, {
          headers: this.$store.getters.getAuth,
        }).then((res) => {
