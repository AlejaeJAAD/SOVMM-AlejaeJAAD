    ...
    <v-form>
      <v-row>
        <v-col cols="12">
          <a @click="styleButton" style="color: inherit; text-decoration: none;"
             href="https://sovmm-03.s3.us-east-2.amazonaws.com/FORMATO-EVALUACION+Y+SEGUIMIENTO+DE+RESIDENCIA+PROFESIONAL.pdf"
             target="_blank">
            <v-btn
              large
              dark
          ...
          <form action="https://sovmm-03.s3.us-east-2.amazonaws.com/FORMATO-EVALUACION+Y+SEGUIMIENTO+DE+RESIDENCIA+PROFESIONAL.pdf"
                method="get">
          </form>
        </v-col>
        <v-col cols="12">
          ...
          <document-handler single v-model="primerSeguimientoEv"></document-handler>
        </v-col>
      </v-row>
      <v-card-actions>
        <v-spacer></v-spacer>
      ...
      <v-row>
        <v-col cols="12">
          <v-card flat
                  align="center"
                  v-for="document in this.info.primerSeguimientoEv" :key="document">
            <pdf :src="document"
                 :page="currentPage"
...
export default {
  ...
  methods: {
    ...
    postDocument() {
      console.log(this.primerSeguimientoEv, 'ih');
      console.log(this.proy.primerSeguimientoEv, '49494');
      /*
      const data = {
        document: '',
      };
      ...
      */
      if (this.primerSeguimientoEv.length !== 0) {
        console.log('entro0');
        ...
        this.isLoading = true;
        const fD = new FormData();
        for (let i = 0; i < this.primerSeguimientoEv.length; i += 1) {
          fD.append(`document${i}`, this.primerSeguimientoEv[i]);
        }
        Axios.post('documents', fD, {
          headers: this.$store.getters.getAuth,
        }).then((res) => {
          ...
          proyect.primerSeguimientoEv = res.data.docs;
          proyect.status = true;
