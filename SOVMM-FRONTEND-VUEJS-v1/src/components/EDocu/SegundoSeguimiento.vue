  ...
  <v-card flat style="margin-bottom: -0.5rem">
    <v-form>
      <v-row>
        ...
        <v-col cols="12">
          <v-card-subtitle>
            Agrega tu segundo seguimiento
            <v-icon>mdi-arrow-down-bold</v-icon>
          </v-card-subtitle>
          <document-handler single v-model="segundaSeguimientoEv"></document-handler>
        </v-col>
      </v-row>
      <v-card-actions>
        <v-spacer></v-spacer>
      ...
      <v-row>
        <v-col cols="12">
          <v-card flat
                  align="center"
                  v-for="document in this.info.segundaSeguimientoEv" :key="document">
            <pdf :src="document"
                 :page="currentPage"
    ...
    postDocument() {
      ...
      */
      if (this.segundaSeguimientoEv.length !== 0) {
        ...
        this.isLoading = true;
        const fD = new FormData();
        for (let i = 0; i < this.segundaSeguimientoEv.length; i += 1) {
          fD.append(`document${i}`, this.segundaSeguimientoEv[i]);
        }
        Axios.post('documents', fD, {
          headers: this.$store.getters.getAuth,
        }).then((res) => {
