  ...
  <div>
    ...
    >
      ...
      <template v-slot:append>
        <div class="pa-2">
          <v-btn color="black" block outlined
                 @click="$store.dispatch('logOut').then($router.push('/login'))">
            Cerrar Sesion <v-icon>mdi-logout</v-icon>
          </v-btn>
        </div>
      </template>
    </v-navigation-drawer>
...
export default {
  ...
  methods: {
    search() {
      ...
      this.$router.replace({ path: this.$route.path, query: { s: this.searchQuery } })
        .catch(() => ({ s: this.searchQuery }));
      this.searchQuery = '';
    },
    cancelQuery() {
    ...
    items() {
      ...
      if (this.$store.getters.getUser.accessLevel === 3) {
        items.push(
          ...
          {
            title: 'Mi proyecto de residencias',
            icon: 'mdi-home-city',
            to: '/mi-proyecto-residencias',
            hovered: false,
          },
        );
      }
