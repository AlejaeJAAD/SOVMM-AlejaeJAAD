<template>
  <v-container>
    <v-row justify="center">
      <v-col cols="12">
        <v-dialog v-model="creacionEstudiante" persistent max-width="600px">
          <v-card>
            <v-card-title>
              <v-spacer />
              <div class="text-center">
                <span class="text-h5">CREAR ESTUDIANTE</span>
              </div>
              <v-spacer />
            </v-card-title>
            <v-divider></v-divider>
            <v-card-text>
              <v-container>
                ...
                <v-row style="justify-content: space-around">
                  ...
                  <v-col cols="12" style="margin: -2rem 0 -2rem 0">
                    <v-text-field dense label="Numero de control" single-line outlined :disabled="toUpdate" counter maxlength="8" id="Numerodecontrol" @keypress="checkNumber($event)" ref="inpNumerodecontrol" v-model="numerodecontrol"></v-text-field>
                  </v-col>
                  <v-col cols="12">
...
export default {
  ...
  data() {
    return {
      ...
      correoinstitucionalSuffix: "@itculiacan.edu.mx"
    };
  },
  created() {
    console.log(this.creacionEstudiante);
  },
  watch: {
    numerodecontrol(val) {
      const nControl = val;
  ...
  methods: {
    ...
    registrar() {
      ...
      if (this.checaCamposVacio()) {
        ...
        console.log(estudiante.birthDate, estudiante.userSince);
