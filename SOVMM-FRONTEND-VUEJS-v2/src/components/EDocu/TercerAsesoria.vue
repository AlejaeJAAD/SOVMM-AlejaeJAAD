  ...
  <v-card flat style="margin-bottom: -0.5rem">
    <v-form>
      <v-row>
        <v-col cols="12">
          <a @click="styleButton" style="color: inherit; text-decoration: none;" href="https://sovmm-03.s3.us-east-2.amazonaws.com/FORMATO-REGISTRO+DE+ASESORIA.pdf" target="_blank">
            <v-btn large dark @click="styleButton" :outlined="!clicked" :color="!clicked ? 'grey darken-3' : 'black'" :class="!clicked ? 'white--text' : ''">
              Descarga el formato de Tercer Asesoria
        ...
        <v-col cols="12">
          <v-card-subtitle>
            Agrega tu tercer asesoria
            <v-icon>mdi-arrow-down-bold</v-icon>
          </v-card-subtitle>
          <document-handler single v-model="tercerAsesoria"></document-handler>
        </v-col>
      </v-row>
      <v-card-actions>
        <v-spacer></v-spacer>
      ...
      <v-card flat style="background-color: #333333" class="text-center white--text mt-5">
        ...
        CARGAR TERCER ASESORIA DE RESIDENCIAS PROFESIONALES CORRESPONDIENTE PARA SU ACTUALIZACION
      </v-card>
      <v-row>
        <v-col cols="12">
          <v-card flat align="center" v-for="document in this.info.tercerAsesoria" :key="document">
            <pdf
              :src="document"
              :page="currentPage"
...
export default {
  ...
  methods: {
    ...
    postDocument() {
      console.log(this.tercerAsesoria, "ih");
      console.log(this.proy.tercerAsesoria, "49494");
      /*
      const data = {
        document: '',
      };
      ...
      */
      if (this.tercerAsesoria.length !== 0) {
        console.log("entro0");
        ...
        this.isLoading = true;
        const fD = new FormData();
        for (let i = 0; i < this.tercerAsesoria.length; i += 1) {
          fD.append(`document${i}`, this.tercerAsesoria[i]);
        }
        Axios.post("documents", fD, {
          headers: this.$store.getters.getAuth
        }).then(res => {
          ...
          proyect.tercerAsesoria = res.data.docs;
          proyect.status = true;
