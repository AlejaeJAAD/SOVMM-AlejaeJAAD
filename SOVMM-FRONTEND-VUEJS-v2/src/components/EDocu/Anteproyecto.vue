<template>
  <v-card flat style="margin-bottom: -0.5rem">
    <v-form>
      <v-row>
        <v-col cols="12">
          <a @click="styleButton" style="color: inherit; text-decoration: none;" href="https://sovmm-03.s3.us-east-2.amazonaws.com/FORMATO-ANTEPROYECTO+DE+RESIDENCIAS+PROFESIONALES.pdf" target="_blank">
            <v-btn large dark @click="styleButton" :outlined="!clicked" :color="!clicked ? 'grey darken-3' : 'black'" :class="!clicked ? 'white--text' : ''">
              Descarga el formato de Anteproyecto de Residencias Profesionales
              <v-icon class="ml-2">mdi-download</v-icon>
            </v-btn>
          </a>
          <form action="https://sovmm-03.s3.us-east-2.amazonaws.com/FORMATO-ANTEPROYECTO+DE+RESIDENCIAS+PROFESIONALES.pdf" method="get"></form>
        </v-col>
        <v-col cols="12">
          <v-card-subtitle>
            Agrega tu anteproyecto
            <v-icon>mdi-arrow-down-bold</v-icon>
          </v-card-subtitle>
          <document-handler single v-model="anteProy"></document-handler>
        </v-col>
      </v-row>
      <v-card-actions>
        <v-spacer></v-spacer>
      ...
      <v-card flat style="background-color: #333333" class="text-center white--text mt-5">
        ...
        CARGAR ANTEPROYECTO DE RESIDENCIAS PROFESIONALES CORRESPONDIENTE PARA SU ACTUALIZACION
      </v-card>
      <v-row>
        <v-col cols="12">
          <v-card flat align="center" v-for="document in this.info.anteProy" :key="document">
            <pdf
              :src="document"
              :page="currentPage"
            ...
            <v-col cols="12">
              <a @click="styleButton" style="color: inherit; text-decoration: none;" :href="document" target="_blank">
                <v-btn large dark @click="styleButton" :outlined="!clicked" :color="!clicked ? 'grey darken-3' : 'black'" :class="!clicked ? 'white--text' : ''">
                  Descargar o visualizar en otra pestaña el documento actualizado
                  <v-icon class="ml-2">mdi-download</v-icon>
                </v-btn>
              </a>
            </v-col>
...
export default {
  ...
  methods: {
    ...
    postDocument() {
      console.log(this.anteProy, "ih");
      console.log(this.proy.anteProy, "49494");
      /*
      const data = {
        document: '',
      };
      ...
      */
      if (this.anteProy.length !== 0) {
        ...
        this.isLoading = true;
        const fD = new FormData();
        for (let i = 0; i < this.anteProy.length; i += 1) {
          fD.append(`document${i}`, this.anteProy[i]);
        }
        Axios.post("documents", fD, {
          headers: this.$store.getters.getAuth
        }).then(res => {
          ...
          proyect.anteProy = res.data.docs;
          proyect.status = true;
