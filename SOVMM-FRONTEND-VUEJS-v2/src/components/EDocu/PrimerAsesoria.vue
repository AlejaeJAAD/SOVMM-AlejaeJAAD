<template>
  <v-card flat style="margin-bottom: -0.5rem">
    <v-form>
      <v-row>
        <v-col cols="12">
          <a @click="styleButton" style="color: inherit; text-decoration: none;" href="https://sovmm-03.s3.us-east-2.amazonaws.com/FORMATO-REGISTRO+DE+ASESORIA.pdf" target="_blank">
            <v-btn large dark @click="styleButton" :outlined="!clicked" :color="!clicked ? 'grey darken-3' : 'black'" :class="!clicked ? 'white--text' : ''">
              Descarga el formato de Primer Asesoria
              <v-icon class="ml-2">mdi-download</v-icon>
            </v-btn>
          </a>
          <form action="https://sovmm-03.s3.us-east-2.amazonaws.com/FORMATO-REGISTRO+DE+ASESORIA.pdf" method="get"></form>
        </v-col>
        <v-col cols="12">
          <v-card-subtitle>
            Agrega tu primer asesoria
            <v-icon>mdi-arrow-down-bold</v-icon>
          </v-card-subtitle>
          <document-handler single v-model="primerAsesoria"></document-handler>
        </v-col>
      </v-row>
      <v-card-actions>
        <v-spacer></v-spacer>
      ...
      <v-card flat style="background-color: #333333" class="text-center white--text mt-5">
        ...
        CARGAR PRIMER ASESORIA DE RESIDENCIAS PROFESIONALES CORRESPONDIENTE PARA SU ACTUALIZACION
      </v-card>
      <v-row>
        <v-col cols="12">
          <v-card flat align="center" v-for="document in this.info.primerAsesoria" :key="document">
            <pdf
              :src="document"
              :page="currentPage"
...
export default {
  ...
  methods: {
    ...
    postDocument() {
      console.log(this.primerAsesoria, "ih");
      console.log(this.proy.primerAsesoria, "49494");
      /*
      const data = {
        document: '',
      };
      ...
      */
      if (this.primerAsesoria.length !== 0) {
        console.log("entro0");
        ...
        this.isLoading = true;
        const fD = new FormData();
        for (let i = 0; i < this.primerAsesoria.length; i += 1) {
          fD.append(`document${i}`, this.primerAsesoria[i]);
        }
        Axios.post("documents", fD, {
          headers: this.$store.getters.getAuth
        }).then(res => {
          ...
          proyect.primerAsesoria = res.data.docs;
          proyect.status = true;
